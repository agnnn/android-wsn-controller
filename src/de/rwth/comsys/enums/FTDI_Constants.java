...
public final class FTDI_Constants
{
	...
	// So the requestType, request, value, index are all used to send control messages.
	// Below are some pre-defined values.
	...
	public static final int FTDI_DEVICE_IN_REQTYPE = UsbConstants.USB_TYPE_VENDOR | USB_RECIP_DEVICE
	...
	/* pre-defined Index: */
	// The FTDI channel(interface) is the most commonly used as Index parameter in usb
	// control message tranmitting.
	// Different sources designates different index for interfaces. Some says it begins
	// with 0 for A and ends 3 for D.
	// While some says it begin with 1 for A and 4 for D. After a while of searching, I
	// tend to believe it 1 for A and 4 for D.
	...
	// flow control setting is a little special, the following values are actually sent in
	// "Index"
	// while higher byte is flow control, lower byte is chip channel index (channel A, B,
	// C or D)
	...
	// So i didn't list more here. And I let the program claim error when the setting is
	// not 7 or 8.
	// But Microsoft's SerialPort class does support data bits = 5. Need to gether more
	// information on this.
