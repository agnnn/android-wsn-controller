...
/**
 * Used to create packets for MSP430 device (serial standard protocol). For details have a
 * look on SLAU319B.pdf.
 * 
...
public class MSP430PacketFactory
{
	...
	/**
	 * Creates a MSP430 RX PASSWORD command. 80 10 24 24 xx xx xx xx D1 D2 … D20 CKL CKH
	 * ACK
	 * 
	 * @param password
	...
	/**
	 * Creates a MSP430 TX BSL VERSION command, which invokes the MSP430 to respond with
	 * data from startAddress until startAddress + length (16-bit blocks). TX BSL version
	 ...
	 * data from startAddress until startAddress + length (16-bit blocks). SLAU319B.pdf TX
	 * DATA BLOCK 80 14 04 04 AL AH LL LH CKL CKH
	 * 
	 * @param startAddressHighByte
	 * @param startAddressLowByte
	 * @param length How many 16-bit blocks shall be read? TODO Limited to 20 blocks (FTDI
	 *        endpoint 64 byte)? , 16 best w.r.t I16HEX!
	...
	public static byte[] createTXDataBlockCommand(short startAddressHighByte, short startAddressLowByte, short length)
	{
		...
		result[0] = 0x80; // HEADER
		result[1] = 0x14; // CMD
		result[2] = 04; // L1: Number of bytes consisting of AL through Dn. Restrictions:
						// L1 = L2, L1 < 250, L1 even
		result[3] = result[2]; // L2:= L1
	...
	/**
	 * Creates a MSP430 LOAD PC packet. SLAU319B.pdf Load PC 80 1A 04 04 AL AH x x CKL CKH
	 * ACK
	 * 
	...
	public static byte[] createLoadPCCommand(short startAddressHighByte, short startAddressLowByte)
	{
		...
		result[0] = 0x80; // HEADER
		result[1] = 0x1A; // CMD
		result[2] = 0x04; // L1: Number of bytes consisting of AL through Dn.
							// Restrictions: L1 = L2, L1 < 250, L1 even
		result[3] = result[2]; // L2:= L1
		result[4] = startAddressLowByte; // AL: start address
		result[5] = startAddressHighByte; // AH
	...
	/**
	 * Creates a CHANGE BAUDRATE packet. SLAU319B.pdf Load PC 80 1A 04 04 AL AH x x CKL
	 * CKH ACK
	 * 
	 * @param data to write must be have even length <= 52 and > 4
	 * @param startAddress 16-bit address < 1FF
	...
	public static byte[] createChangeBaudrateCommand(FTDI232BM_Matching_MSP430_Baudrates baudrate, MSP430Variant variant)
	{
		...
		/*
		 * switch(variant) { case MSP430_F149: switch (baudrate) { case BAUDRATE_9600: al
		 * = 0x80; ah = 0x85; ll = 0x00; break; case BAUDRATE_19200: al = 0xE0; ah = 0x86;
		 * ll = 0x01; break; case BAUDRATE_38400: al = 0xE0; ah = 0x87; ll = 0x02; break;
		 * default: return null; } break; case MSP430_F449: switch (baudrate) { case
		 * BAUDRATE_9600: al = 0x00; ah = 0x98; ll = 0x00; break; case BAUDRATE_19200: al
		 * = 0x00; ah = 0xB0; ll = 0x01; break; case BAUDRATE_38400: al = 0x00; ah = 0xC8;
		 * ll = 0x02; break; default: return null; } break; case MSP430_F2131: switch
		 * (baudrate) { case BAUDRATE_9600: al = 0x80; ah = 0x85; ll = 0x00; break; case
		 * BAUDRATE_19200: al = 0x00; ah = 0x8B; ll = 0x01; break; case BAUDRATE_38400: al
		 * = 0x80; ah = 0x8C; ll = 0x02; break; default: return null; } break; default:
		 * return null;
		 * 
		 * }
		 */
	...
	/**
	 * Creates a MSP430 RX Data block packet. SLAU319B.pdf RX data block 80 12 n n AL AH
	 * n-4 0 D1 D2 ... Dn-4 CKL CKH ACK
	 * 
	 * @param data to write must be have even length <= 52 and > 4
	 * @param startAddressHighByte
	 * @param startAddressLowByte
	...
	public static byte[] createRXDataBlockCommand(short[] data, short startAddressHighByte, short startAddressLowByte)
	{
		...
		// check length
		if (((data.length > 52) || (data.length < 4)) && (data.length % 2 == 1))
			return null;
		...
		result[0] = 0x80; // HEADER
		result[1] = 0x12; // CMD
		result[2] = (short) (data.length + 4); // L1: Number of bytes consisting of AL
												// through Dn. Restrictions: L1 = L2, L1 <
												// 250, L1 even
		result[3] = result[2]; // L2:= L1
		result[4] = startAddressLowByte; // AL: start address
		result[5] = startAddressHighByte; // AH
	...
	/**
	 * Creates a MSP430 Mass Erase command packet, which erases the entire flash memory
	 * area. SLAU319B.pdf MASS ERASE 80 18 04 04 xx xx 06 A5 CKL CKH ACK
	 * 
	 * @return packet to send
	 */
	...
	/**
	 * Calculates CKL value for a MSP430 packet. SLAU319B.pdf
	 * 
	...
	/**
	 * Calculates CKH value for a MSP430 packet. SLAU319B.pdf
	 * 
	 * @param data complete packet with ckl, ckh, ack
	 * @return ckh
	 */
