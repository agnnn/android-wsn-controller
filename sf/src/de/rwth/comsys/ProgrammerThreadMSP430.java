	...
	public void run() {
		...
		for (MSP430Command cmd : commands) {
			...
			case CHANGE_BAUDRATE:
				...
				if(ftdiInterface.setBaudrate(FTDI232BM_Matching_MSP430_Baudrates.BAUDRATE_38400))
				{
					IOHandler.doOutput("successfully set baudrate to "+FTDI232BM_Matching_MSP430_Baudrates.BAUDRATE_38400+" on ftdi");
				}
				sendResetSequence(false); // simply restart mote
				break;
		...
		if (resp.length > 0) {
			...
			else
			{
				for (int i = 0; i < resp.length; i++) {
					byte b = resp[i];
					IOHandler.doOutput("err-answer: 0x"+Integer.toHexString(b));
				}
