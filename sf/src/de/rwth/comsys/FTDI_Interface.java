		...
		try {
			...
			do {
				...
				if (curRead != -1) {
					if(curRead > 2)
					{
						overallBuffer.put(buffer, overallRead, curRead-2); // TODO offset was 2
			...
			do {
				...
				if (curRead != -1) {
					if (curRead > 2) // more than the ftdi head bytes received?
					{
						if ((buffer[2] & 0xFF) != 0) 
						{
							IOHandler.doOutput("bytesRead: "+curRead);
						...
						else
						{
							for (byte b : buffer) {
								if(b != 0)
									IOHandler.doOutput("received: 0x"+Integer.toHexString(b));
							}
						}
					}
				}
